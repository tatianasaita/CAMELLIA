...
print.cluster_dendrogram_result <- function(x, ...) {
  # MODIFICADO: Adicionar informação do método
  method_name <- if (!is.null(x$method)) {
    switch(x$method,
           "hierarchical" = "Hierarchical Clustering",
           "affinity_propagation" = "Affinity Propagation",
           "Unknown Method")
  } else {
    "Hierarchical Clustering"  # Default para compatibilidade
  }
  ...
  # NOVO: Informações específicas do AP
  if (!is.null(x$method) && x$method == "affinity_propagation") {
    cat(sprintf("Number of exemplars: %d\n", length(x$exemplars)))
    if (!is.null(x$ap_result)) {
      cat(sprintf("Iterations:         %d\n", x$ap_result@iterations))
    }
  }
...
print.train_models_rf_xgboost <- function(x, ...) {
  ...
  cat("  Classes:", nlevels(x$actuals_validation), "\n\n")
  cat("Performance comparison (Validation set):\n")
  print(x$model_comparison)
  cat("\nBest model (validation):", x$best_model_validation, "\n")
  cat("Best model (test ext.) :", x$best_model_test_ext,   "\n")
  invisible(x)
}
...
print.kmer_analysis_result <- function(x, ...) {
  ...
  if (x$has_external_test) {
    cat("\nTEST DATA:\n")
    cat(sprintf("  Class-specific motifs:    %d\n", nrow(x$test$unique_class_motifs)))
...
print.seq_classification <- function(x, ...) {
  ...
  cat("  Clustering method :", x$parameters$cluster_method, "\n")
