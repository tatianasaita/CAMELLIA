...
#'
 .createNet_fast <- function(word, step, sequence, all_vertices = NULL) {
   ...
   net <- graph(edges = edges_vec, directed = TRUE)
   
   if (!is.null(all_vertices)) {
     missing_v <- setdiff(all_vertices, V(net)$name)
     if (length(missing_v) > 0) {
       net <- add_vertices(net, length(missing_v), name = missing_v)
     }
   }
...
.select_by_cluster_fast <- function(motif_cluster, cluster_to_class, classe_order, n, m,
  ...
  # Seleciona motifs base para cada cluster
  ...
  if (remainder > 0 && !is.null(cluster_homogeneity)) {
    ...
    # cluster_homogeneity: named numeric vector — names == colunas de motif_cluster
    valid_hom <- cluster_homogeneity[names(cluster_homogeneity) %in% names(cluster_to_class)]
    
    if (length(valid_hom) == 0) {
      warning(".select_by_cluster_fast: 'cluster_homogeneity' não tem nomes correspondentes aos clusters — resto ignorado.")
    } else {
      # Ordena clusters do menos homogêneo para o mais homogêneo
      clusters_by_hom <- names(sort(valid_hom, decreasing = FALSE))
      
      for (i in seq_len(remainder)) {
        col    <- clusters_by_hom[[i]]           # i-ésimo cluster na ordem crescente
        classe <- cluster_to_class[col]
        values <- motif_cluster[, col, drop = TRUE]
    ...
    warning(".select_by_cluster_fast: 'cluster_homogeneity' não fornecido — resto não será distribuído.")
  }
